...
var BundleAnalyzerPlugin = function () {
  ...
  _createClass(BundleAnalyzerPlugin, [{
    ...
    value: function generateStatsFile(stats) {
      ...
      return bfj.write(statsFilepath, stats, options).then(function () {
        _this2.logger.info(`${bold('Webpack Bundle Analyzer')} saved stats file to ${bold(statsFilepath)}`);
      }).catch(function (error) {
        _this2.logger.error(`${bold('Webpack Bundle Analyzer')} error saving stats file to ${bold(statsFilepath)}.`, JSON.stringify(error, null, "\n") // eslint-disable-line quotes
        );
      });
    }
  }, {
